...
public class FarmTrollheimHerbs {
    public static int farmTrollheimHerbs(HerbPatchesEnum patch) {
        if(Inventory.isItemSelected()) Inventory.deselect();
        PatchStatus patchStatus = getTrollheimPatchStatus();
        ...
        if(patchStatus == PatchStatus.HARVEST) {
            ...
            Sleep.sleepUntil(() -> Inventory.isFull() ||  getTrollheimPatchStatus() != PatchStatus.HARVEST, 25000);
            if(getTrollheimPatchStatus() == PatchStatus.HARVEST) return 0;
            patch.resetPatch();
            if (getTrollheimPatchStatus() == PatchStatus.HARVEST) return 150;
        ...
        if(patchStatus == PatchStatus.RAKE) {
            ...
            getTrollheimPatch().interact();
            Sleep.sleepUntil(() -> Inventory.isFull() ||  getTrollheimPatchStatus() != PatchStatus.RAKE, 25000);
        ...
        if(patchStatus == PatchStatus.CLEAR) {
            ...
            getTrollheimPatch().interact();
            Sleep.sleepUntil(() -> Inventory.isFull() ||  getTrollheimPatchStatus() != PatchStatus.CLEAR, 25000);
        }
        if(patchStatus == PatchStatus.CURE) {
            ...
            getTrollheimPatch().interact();
            Sleep.sleepUntil(() -> Inventory.isFull() ||  getTrollheimPatchStatus() != PatchStatus.CURE, 25000);
        ...
        if(patchStatus == PatchStatus.PLANT) {
            ...
            if(playerHasCompost && patchIsNotTreated) {
                ...
                if(Dialogues.inDialogue()) {
                    Logger.log("Is in dialogue, planting failed");
                    Logger.log("Dialogue text: " + Dialogues.getNPCDialogue());
                    Logger.log("Dialogue: ");
            ...
            if(getTrollheimPatchStatus() == PatchStatus.PLANTED || seedsCount > Inventory.count(getSettings().seedsToPlant.getHerbSeeds().getSeedName())) {
    ...
    public static PatchStatus getTrollheimPatchStatus() {

//        int bitValue = getSettings().seedsToPlant.getHerbSeeds() == HerbSeeds.AVANTOE_SEED ? PlayerSettings.getBitValue(4954) : PlayerSettings.getBitValue(4771);
