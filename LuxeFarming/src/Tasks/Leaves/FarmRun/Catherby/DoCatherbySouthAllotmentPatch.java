...
public class DoCatherbySouthAllotmentPatch extends Leaf {
    @Override
    public boolean isValid() {
        PatchStatus patchStatus = AllotmentPatchEnum.CATHERBY_ALLOTMENT_PATCH_SOUTH.getPatchStatus();
        boolean isPlanted = patchStatus == PatchStatus.PLANTED;
        if(isPlanted && getSettings().catherby_allotment_south_tp == 0) {
            getSettings().catherby_allotment_south_tp = System.currentTimeMillis();
    ...
    @Override
    public int onLoop() {
        return FarmAllotment(AllotmentPatchEnum.CATHERBY_ALLOTMENT_PATCH_SOUTH);
    }
}
