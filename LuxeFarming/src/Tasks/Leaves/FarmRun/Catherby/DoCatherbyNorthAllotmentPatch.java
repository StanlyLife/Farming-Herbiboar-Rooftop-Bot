...
public class DoCatherbyNorthAllotmentPatch extends Leaf {
    @Override
    public boolean isValid() {
        PatchStatus patchStatus = AllotmentPatchEnum.CATHERBY_ALLOTMENT_PATCH_NORTH.getPatchStatus();
        boolean isPlanted = patchStatus == PatchStatus.PLANTED;
        if(isPlanted && getSettings().catherby_allotment_north_tp == 0) {
            getSettings().catherby_allotment_north_tp = System.currentTimeMillis();
    ...
    @Override
    public int onLoop() {
        return FarmAllotment(AllotmentPatchEnum.CATHERBY_ALLOTMENT_PATCH_NORTH);
    }
}
