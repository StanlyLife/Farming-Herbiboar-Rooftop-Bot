...
public class DoCanafisSouthAllotmentPatch extends Leaf {
    @Override
    public boolean isValid() {
        PatchStatus patchStatus = AllotmentPatchEnum.CANAFIS_ALLOTMENT_PATCH_SOUTH.getPatchStatus();
        boolean isPlanted = patchStatus == PatchStatus.PLANTED;
        if(isPlanted && getSettings().canafis_allotment_south_tp == 0) {
            getSettings().canafis_allotment_south_tp = System.currentTimeMillis();
    ...
    @Override
    public int onLoop() {
        return FarmAllotment(AllotmentPatchEnum.CANAFIS_ALLOTMENT_PATCH_SOUTH);
    }
}
